...
term6 = 19
term7 = 22  # and so on...
We can easily obtain the next terms of the sequence following the 
values in the pattern list. We see that the sixth term of the sequence, 
19, has the sum of its digits 10.
...
--main = print $ [1] !! 0
main = print $ sumDigNthTerm 10 [2, 1, 3] 100000

sumDigNthTerm :: Int -> [Int] -> Int -> Int
sumDigNthTerm dInit dsPattern dNthTerm =
  sumUpDigits (generateSequence dInit dsPattern 1 0 dNthTerm) 0

generateSequence :: Int -> [Int] -> Int -> Int -> Int -> Int
generateSequence dCurrentNumber dsPattern dCurrentTerm dPatternIndex dNthTerm
  | dPatternIndex >= length dsPattern = generateSequence dCurrentNumber
                                                         dsPattern
                                                         dCurrentTerm
                                                         0
                                                         dNthTerm
  | dCurrentTerm == dNthTerm = dCurrentNumber
  | dCurrentTerm < dNthTerm = generateSequence
    (dCurrentNumber + (dsPattern !! dPatternIndex))
    dsPattern
    (dCurrentTerm + 1)
    (dPatternIndex + 1)
    dNthTerm

sumUpDigits :: Int -> Int -> Int
sumUpDigits d dSum
  | d `div` 10 == 0 = dSum + d `mod` 10
  | d `div` 10 > 0  = sumUpDigits (d `div` 10) (dSum + d `mod` 10)
