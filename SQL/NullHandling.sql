...
SELECT eusales.id, COALESCE(NULLIF(eusales.name, ''), '[product name not found]') AS name, NULLIF(eusales.price, 50) AS price, COALESCE(NULLIF(eusales.card_name, ''), '[card name not found]') AS card_name, eusales.card_number, eusales.transaction_date
FROM eusales
WHERE eusales.price > 50;