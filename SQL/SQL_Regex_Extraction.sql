...
-- Regex to create the user_id column
-- Regex -> Search for # and then seen if there are numbers after it, which identifies it as a user_id
SELECT greetings.name, greetings.greeting, substring(greetings.greeting FROM '^.*#([0-9]+).*$') AS user_id
FROM greetings